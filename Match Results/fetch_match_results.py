...
def initialize_data():
    ...
    if not os.path.exists(WEBSCRAPE_MATCHES_FILE):
        ...
        date = pd.to_datetime("1986-1-2") # Initialize with first date of data if no data is available
...
def setup_webdriver(date):
    ...
    """
    logging.info(f"Setting up webdriver for {date.date()}...")
...
def scrape_data(df, browser, date):
    ...
    while not finished_scraping:
        date = pd.to_datetime(date + timedelta(days = 1)) # Fetch next date
        ...
        if len(elements) > 0: # If there are any match results on the page
            for element in elements:
                try:
                    ...
                    pass
            logging.info(f"Finished collecting data for {date.date()}!")
        else:
            logging.info(f"No matches found for {date.date()}.")

        if date.day in [1, 11, 21]: # Periodically save data
            df.drop_duplicates().reset_index(drop = True).to_parquet(WEBSCRAPE_MATCHES_FILE) # Save data to parquet file once finished
            logging.info(f"Saved data on {date.date()}.")
...
def main():
    ...
    df.drop_duplicates().reset_index(drop = True).to_parquet(WEBSCRAPE_MATCHES_FILE) # Save data to parquet file once finished
    date = pd.to_datetime(df.date.max())
    logging.info(f"Finished scraping and saved data on {date.date()}.")
